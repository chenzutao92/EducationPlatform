    ...
    def post(self, request):
        ...
        if register_forms.is_valid():
            ...
            if UserProfile.objects.get(email=user_name):
                return render(request, 'register.html', {'msg': '用户已经存在', 'register_forms': register_forms})
            pass_word = request.POST.get('password', '')
...
class ModifyPwdView(View):
    ...
    def post(self, request):
        ...
        if modify_form.is_valid():
            ...
            email = request.POST.get('email', '')
            if pwd1 != pwd2:
                render(request, 'password_reset.html', {'email': email, 'msg': '密码不一致'})
            user = UserProfile.objects.get(email=email)
...
class MyFavOrgView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        for fav_org in fav_orgs:
            # fav_id 是机构的ID，收藏时会把id 传给fav_id
